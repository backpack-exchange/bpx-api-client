...
mod tests {
    ...
    fn order_update_type_wire() {
        ...
        assert_wire(&OrderUpdateType::Speedbumped, "speedbumped");
        assert_wire(&OrderUpdateType::ReleaseFailed, "releaseFailed");
        assert_wire(
    ...
    fn test_order_update_speedbump() {
        let data = r#"
        {"E":1748288615134547,"O":"USER","Q":"3568.3445","S":"Ask","T":1748288615133255,"V":"RejectTaker","X":"PendingRelease","Z":"0","e":"speedbumped","f":"GTC","i":"114575842681290753","o":"LIMIT","p":"178.15","q":"20.03","r":false,"s":"SOL_USDC","t":null,"z":"0"}
        "#;
        let order_update: OrderUpdate = serde_json::from_str(data).unwrap();
        assert_eq!(order_update.event_type, OrderUpdateType::Speedbumped);
        assert_eq!(order_update.order_status, OrderStatus::PendingRelease);

        let data = r#"
        {"E":1748288615134547,"O":"USER","Q":"3568.3445","R":"OrderBookClosed","S":"Ask","T":1748288615133255,"V":"RejectTaker","X":"Expired","Z":"0","e":"releaseFailed","f":"GTC","i":"114575842681290753","o":"LIMIT","p":"178.15","q":"20.03","r":false,"s":"SOL_USDC","t":null,"z":"0"}
        "#;
        let order_update: OrderUpdate = serde_json::from_str(data).unwrap();
        assert_eq!(order_update.event_type, OrderUpdateType::ReleaseFailed);
        assert_eq!(order_update.order_status, OrderStatus::Expired);
        assert_eq!(
            order_update.order_expiry_reason.as_deref(),
            Some("OrderBookClosed")
        );
    }
